...
const updateStep = (id) => {
    tabs.forEach(tab => {
        tab.classList.contains("tab-active") && tab.classList.remove("tab-active");
    });
    steps.forEach(step => {
        step.classList.contains("step-active") && step.classList.remove("step-active");
    });
    let currentActiveTab = document.getElementById(id);
    currentActiveTab.classList.add("tab-active");

    // Checking which is the current id and setting the step accordingly
    if (id === "tab-1") {
...
const searchByTabs = document.querySelectorAll(".search-tab");
const searchByOptions = document.querySelectorAll(".search-by-option");

const getOptionIdName = id => {
    updateSearchOption(id);
    return id;
};

const updateSearchOption = (id) => {
    searchByTabs.forEach(tab => {
        tab.classList.contains("search-tab-active") && tab.classList.remove("search-tab-active");
    });
    searchByOptions.forEach(option => {
        option.classList.contains("search-by-active") && option.classList.remove("search-by-active");
    });
    let currentActiveTab = document.getElementById(id);
    currentActiveTab.classList.add("search-tab-active");

    // Checking which is the current id and setting the step accordingly
    if (id === "search-by-tab-1") {
        searchByOptions[0].classList.add("search-by-active");
    }
    if (id === "search-by-tab-2") {
        searchByOptions[1].classList.add("search-by-active");
    }
    if (id === "search-by-tab-3") {
        searchByOptions[2].classList.add("search-by-active");
    }
};
